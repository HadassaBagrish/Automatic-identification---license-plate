...
while ret:
    ...
    if ret:
        ...
        # cv2.waitKey(0)
# Convert the validation list to a DataFrame
validation_df = pd.DataFrame.from_dict(validation, orient='index')

# Save the DataFrame to a CSV file
header = ['frame_nmr', 'car_bbox', 'license_number']

validation_df.to_csv('validation_data1.csv', columns=header, index=False)
out.release()
cap.release()
